package com.zp.androidx.picker.camera;
...
import com.zp.androidx.picker.R;
import com.zp.androidx.picker.crop.CropView;
import com.zp.androidx.picker.crop.FrameOverlayView;
import com.zp.androidx.picker.util.FileUtil;
...
public class CameraActivity extends Activity {
    ...
    //从相册进行选择照片
    public static final String KEY_PICK_FROM_ALBUM = "pickFromAlbum";
    ...
    public static void open(Activity activity, int requestCode, String type, String outPutPath){
        open(activity, requestCode, type, outPutPath, false);
    }

    public static void open(Activity activity, int requestCode, String type, String outPutPath, boolean isFromAlbum){
        Intent intent = new Intent(activity, CameraActivity.class);
        if(TextUtils.isEmpty(type)) type = CONTENT_TYPE_GENERAL;
        intent.putExtra(CameraActivity.KEY_CONTENT_TYPE, type);

        intent.putExtra(KEY_PICK_FROM_ALBUM, isFromAlbum);

        if(TextUtils.isEmpty(outPutPath)) {
            String fileName = DateUtils.formatDateTime(activity, System.currentTimeMillis(), DateUtils.FORMAT_SHOW_DATE|DateUtils.FORMAT_SHOW_TIME)+"_"+type+".jpg";
            outPutPath = FileUtil.getSaveFile(activity.getApplication(), fileName).getAbsolutePath();
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // todo zhaopan 不需要自动识别功能.
        //cameraView.setAutoPictureCallback(autoTakePictureCallback);

        isFromAlbum = getIntent().getBooleanExtra(KEY_PICK_FROM_ALBUM, false);
        if(isFromAlbum){
            albumButtonOnClickListener.onClick(findViewById(R.id.album_button));
        }
    }
    ...
    private void initParams() {
        ...
        isNativeEnable = getIntent().getBooleanExtra(KEY_NATIVE_ENABLE, false);
        isNativeManual = getIntent().getBooleanExtra(KEY_NATIVE_MANUAL, true);
